...
use crate::{BackendRenderContext, RenderContext, RenderContextConfig};
...
pub struct WebRenderContext {
    inner: BackendRenderContext<'static>,
    surface_target: WebSurfaceTarget,
    gpu: Context,
}

impl Deref for WebRenderContext {
    type Target = Context;
...
impl WebRenderContext {
    ...
    pub async fn create(
        ...
        let instance =
            crate::gpu::create_instance_with_wgpu_detection(&wgpu::InstanceDescriptor::default())
                .await;
        ...
        let target = BackendRenderContext::new(&gpu, surface, &render_target_config)?;
...
#[cfg(target_arch = "wasm32")]
impl super::Canvas {
    pub fn new_web(target: &WebRenderContext, config: CanvasConfig) -> Self {
